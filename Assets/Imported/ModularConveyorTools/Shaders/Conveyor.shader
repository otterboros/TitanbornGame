...
Shader "Factory/Conveyor"
{
	Properties
	{
		...
		[ASEBegin]_Speed("Speed", Range( -2 , 2)) = 1
		_Albedo("Albedo", 2D) = "white" {}
		_Normal("Normal", 2D) = "bump" {}
		_Metallic("Metallic", 2D) = "black" {}
		[ASEEnd]_Smoothness("Smoothness", 2D) = "gray" {}
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float _Speed_Instance = UNITY_ACCESS_INSTANCED_PROP(FactoryConveyor,_Speed);
				...
				float2 break33 = texCoord1;
				float4 appendResult34 = (float4(break33.x , ( break33.y * temp_output_38_0 ) , 0.0 , 0.0));
				float2 panner2 = ( _TimeParameters.x * ( _Speed_Instance * appendResult43 ).xy + appendResult34.xy);
				

				float3 BaseColor = tex2D( _Albedo, panner2 ).rgb;
				float3 Normal = UnpackNormalScale( tex2D( _Normal, panner2 ), 1.0f );
				float3 Emission = 0;
				float3 Specular = 0.5;
				float Metallic = tex2D( _Metallic, panner2 ).r;
				float Smoothness = tex2D( _Smoothness, panner2 ).r;
				float Occlusion = 1;
				float Alpha = 1;
...
WireConnection;45;3;9;0
WireConnection;45;4;10;0
ASEEND*/
//CHKSM=3ED983B0739C17560EF9EA73F3D2AE360D788011